...
CONF_C_PRELUDE = CONF_C_LIKE_PRELUDE + '''{}
...
def _dump_js_module(ofile: T.TextIO, cdata: ConfigurationData) -> None:
    ofile.write(CONF_C_LIKE_PRELUDE)
    for k in sorted(cdata.keys()):
        (v, desc) = cdata.get(k)
        if desc:
            ofile.write(f'/* {desc} */\n')
        ofile.write(f'export const {k} = ')
        if isinstance(v, str):
            ofile.write(v)
        else:
            json.dump(v, ofile, sort_keys=True)
        ofile.write(';\n\n')


def dump_conf_header(ofilename: str, cdata: ConfigurationData,
                     output_format: Literal['c', 'esm', 'nasm', 'json'],
                     macro_name: T.Optional[str]) -> None:
    ...
    with open(ofilename_tmp, 'w', encoding='utf-8') as ofile:
        ...
        elif output_format == 'esm':
            _dump_js_module(ofile, cdata)
        else:  # c, nasm
